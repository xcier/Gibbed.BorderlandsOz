...
namespace Gibbed.BorderlandsOz.FileFormats
{
    public class SaveFileJSON
    {
        ...
        public SaveFile ToSaveFile()
        {
            ...
            var save = new WillowTwoPlayerSaveGame
            {
                ...
                // Empty or not found
                ItemData = item_list.Select(i => (ItemData)i).ToList(),
                // Empty
                ...
                // Empty
                WeaponData = weapon_data.Select(w => (WeaponData)w).ToList(),
                // Not found
                ...
                // Not found
                BodySwitches = new List<BodySwitchesData>(),

                // Not found
                PlayerFlags = new List<int>(),
                // Not found
                DiscoveredInteractiveObjectCompassIcons = new List<NameBasedObjectPath>(),
                // Not found
                SuppressAtmosphereNotifications = null,

                QueuedTrainingMessages = field52.ToList(),
        ...
        {
            field52 = saveGame.QueuedTrainingMessages.ToArray();
            item_list = saveGame.ItemData.Select(i => Item_Rewards.From(i)).ToArray();
            weapon_data = saveGame.WeaponData.Select(w => Weapon_Rewards.From(w)).ToArray();
            ...
            field58 = saveGame.VehicleSteeringMode;
        ...
        public Item_Rewards[] item_list { get; set; }
        public Inventory_Data inventory_data { get; set; }
        public Weapon_Rewards[] weapon_data { get; set; }
    ...
    public class Playthrough_Data
    {
        ...
        internal static Playthrough_Data From(MissionPlaythroughData data)
        {
            ...
            {
                filterd = data.FilteredMissions.ToArray(),
                mission_asset = data.ActiveMission,
                number = data.PlayThroughNumber.GetValueOrDefault(),
                mission_data = data.MissionList.Select(m => Mission_Data.From(m)).ToArray(),
        ...
        public static implicit operator MissionPlaythroughData(Playthrough_Data data)
        {
            ...
            {
                FilteredMissions = data.filterd.ToList(),
                PlayThroughNumber = data.number,
                ActiveMission = data.mission_asset,
                MissionList = data.mission_data.Select(m => (MissionData)m).ToList(),
    ...
    public class Saved_Regions
    {
        ...
        public static explicit operator RegionGameStageData(Saved_Regions data)
        {
            return new RegionGameStageData()
            {
                ...
                GameStage = data.game_stage,
                IsFromDLC = data.play_through_idx == 1,
                DLCPackageId = data.field4,
                PlaythroughIndex = data.field5
            };
        }
        ...
        public static Saved_Regions From(RegionGameStageData data)
        {
            return new Saved_Regions()
            {
                ...
                play_through_idx = data.IsFromDLC ? 1 : 0,
                field4 = data.DLCPackageId,
                field5 = data.PlaythroughIndex,
            };
        }
    }
